...
class TestResolveTaskTimeout:
    """Tests for _resolve_task_timeout() config-based timeout resolution."""
    ...
    def test_timeout_resolution_from_yaml(self, tmp_path, monkeypatch, task, config_content, expected):
        ...
        with patch("hermes_cli.config.get_hermes_home", return_value=hermes_home):
            result = _resolve_task_timeout(task)
        assert result == expected

    def test_no_task_returns_default(self):
        """When no task is provided, returns the default timeout."""
        assert _resolve_task_timeout() == 30.0
        assert _resolve_task_timeout(None) == 30.0
        assert _resolve_task_timeout(default_timeout=60.0) == 60.0

    def test_custom_default_timeout(self, tmp_path, monkeypatch):
        """Custom default timeout should be used when config is missing."""
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir(parents=True)
        config_file = hermes_home / "config.yaml"
        ...
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        with patch("hermes_cli.config.get_hermes_home", return_value=hermes_home):
            result = _resolve_task_timeout("vision", default_timeout=60.0)
        assert result == 60.0
